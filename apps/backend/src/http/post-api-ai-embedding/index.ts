import { badRequest, forbidden, internal, methodNotAllowed } from "@hapi/boom";
import {
...
const handlerImpl = async (
  ...
  if (
    ...
    throw badRequest("Text parameter is required and cannot be empty");
  }
  ...
  for (let attempt = 0; attempt < BACKOFF_MAX_RETRIES; attempt++) {
    try {
      ...
      if (!response.ok) {
        ...
        // Not a throttling error or no retries left, throw error
        throw internal(
          `Failed to generate embedding: ${response.status} ${errorText}`
        );
      }
      ...
      if (!data.embedding?.values) {
        ...
        throw internal("Invalid embedding response format");
      }
  ...
  if (lastError) {
    throw internal(
      lastError.message || "Failed to generate embedding: Unknown error"
    );
  }

  throw internal("Failed to generate embedding: Unknown error");
};
