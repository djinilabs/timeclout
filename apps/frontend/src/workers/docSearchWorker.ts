...
async function generateEmbedding(
  ...
  if (!apiUrl || apiUrl.trim().length === 0) {
    throw new Error(
      "Embedding API URL is not configured. Please check your environment settings."
    );
  }
  ...
  try {
    ...
    if (fetchError instanceof Error) {
      if (fetchError.name === "AbortError") {
        throw new Error(
          "Embedding generation timed out. The request took too long to complete."
        );
      }
      if (
        ...
        throw new Error(
          "Unable to connect to the embedding service. Please check your internet connection."
        );
      }
    }
    throw new Error(
  ...
  if (!response.ok) {
    let errorMessage = `Failed to generate embedding: HTTP ${response.status}`;
    ...
    try {
      ...
      try {
        ...
        if (response.status === 401) {
          errorMessage = "Authentication required to generate embeddings.";
        } else if (response.status === 403) {
          errorMessage = "You don't have permission to generate embeddings.";
        } else if (response.status === 429) {
          errorMessage = "Too many embedding requests. Please wait a moment and try again.";
        } else if (response.status >= 500) {
          errorMessage = "Embedding service is temporarily unavailable. Please try again later.";
        }
      }
    }
  ...
  if (!data.embedding) {
    throw new Error("Invalid embedding response format: missing embedding data");
  }

  if (!Array.isArray(data.embedding)) {
    throw new Error("Invalid embedding response format: embedding is not an array");
  }
...
async function performIndexing(apiUrl: string): Promise<void> {
  ...
  const embeddingPromises = documentSnippets.map(
    async ({ documentName, snippetText, snippetHash, documentId }) => {
      try {
        ...
        // Don't throw here - we want to continue indexing other snippets
        // The error will be logged and the indexing will continue
        return {
          success: false,
          documentName,
          error:
  ...
  if (failures.length > 0) {
    console.warn(
      `[performIndexing] Failed to generate embeddings for ${failures.length} snippet(s). Some documents may not be searchable.`
    );
  }
}
...
self.addEventListener("message", async (event: MessageEvent<WorkerMessage>) => {
  ...
  try {
    if (message.type === "search") {
      // Validate query
      if (!message.query || message.query.trim().length === 0) {
        throw new Error("Search query cannot be empty");
      }

      // Validate API URL
      if (!message.apiUrl || message.apiUrl.trim().length === 0) {
        throw new Error(
          "Embedding API URL is not configured. Please check your environment settings."
        );
      }
      ...
      try {
        queryEmbedding = await generateEmbedding(message.query, message.apiUrl);
