    ...
    return (
      <Listbox value={selectedOption} onChange={onValueChange}>
        <div className="relative">
          <ListboxButton
            className="grid w-full cursor-default grid-cols-1 rounded-md bg-white py-1.5 pl-3 pr-2 text-left text-gray-outline-1 -outline-offset-1 outline-gray-300 focus:outline-2 focus:-outline-offset-2 focus:outline-teal-600 sm:text-sm/6"
            aria-label={label}
            aria-expanded={false}
          ...
          >
            {options.map((option) => (
              ...
              >
                ...
                <span
                  className="absolute inset-y-0 right-0 flex items-center pr-4 text-teal-600 group-[&:not([data-selected])]:hidden group-data-focus:text-white"
                  aria-hidden="true"
                >
