...
export const Dialog: FC<PropsWithChildren<DialogProps>> = memo(
  ({ open, title, footer, onClose, children }) => {
    return (
      <HeadlessDialog open={open} onClose={onClose} className="relative">
        ...
        <div className="fixed inset-0 w-screen overflow-y-auto z-[200]">
          <div className="flex min-w-full min-h-full items-end justify-center sm:p-0">
            <DialogPanel
              transition
              className="relative transform overflow-hidden rounded-lg bg-white px-4 pt-5 pb-4 text-left shadow-xl transition-all data-closed:translate-y-4 data-closed:opacity-0 data-enter:duration-300 data-enter:ease-out data-leave:duration-200 data-leave:ease-in sm:my-8 data-closed:sm:translate-y-0 data-closed:sm:scale-95"
            >
